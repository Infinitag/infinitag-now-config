...
Device *DeviceRegistry::byIndex(uint8_t deviceType, size_t idx) {
  // Selection by ascending MAC, stable display order across rescans.
  size_t skip = idx;
  ...
  for (;;) {
    ...
    for (size_t i = 0; i < MAX_DEVICES; i++) {
      ...
      if (!best || memcmp(d.mac, best->mac, 6) < 0) { best = &d; bestIdx = i; }
    }
...
uint32_t DeviceRegistry::maxVersionKey(uint8_t deviceType) const {
  uint32_t maxKey = 0;
  for (const auto &d : _dev) {
    if (!d.used || d.deviceType != deviceType) continue;
    const uint32_t k = versionKey(d.info);
